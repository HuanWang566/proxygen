...
class ControlCenterHandlerFactory : public RequestHandlerFactory {
 public:
  void onServerStart(folly::EventBase* /*evb*/) noexcept override {
    stats_.reset(new ControlCenterStats);
  }
  ...
  RequestHandler* onRequest(RequestHandler*, HTTPMessage*) noexcept override {
    return new ControlCenterHandler(stats_.get());
  }

 private:
  folly::ThreadLocalPtr<ControlCenterStats> stats_;
};
...
int main(int argc, char* argv[]) {
  ...
  options.handlerFactories =
      RequestHandlerChain().addThen<ControlCenterHandlerFactory>().build();
